...
export function getOsintTools(userKeys: { virustotal_api_key?: string } = {}) {
  return {
    search_web: tool({
      ...
      execute: async ({ query }) => {
        try {
          const searchQuery = query || 'WormGPT' // Fallback for proxy bug
          const res = await fetch(`https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(searchQuery)}&utf8=&format=json&srlimit=3`)
